...
@export var cover_move_duration: float = 0.5 # How long it takes to move to the cover spot
...
func update(delta: float) -> void:
	...
	if _is_peeking:
		...
		if _time_since_last_shot_from_cover >= shoot_from_cover_cooldown:
			if enemy_controller.has_method("fire_weapon"):
				var player_target = player.global_transform.origin + Vector3.UP * 1.5 # Aim at player's chest height
	...
	if distance_to_player < enemy_controller.melee_attack_range:
		print("[EnemyCover] Player too close, transitioning to chase.")
